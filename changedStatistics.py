# Description: This is the same file as statistics.py in Ann Christin's Masterthesis code, with an added function readData(filepath) to read in biological spiking data
# Can be used to segment data with quartic polynomial algorithm, and also has statistics-calculating functions
...
# getting excitatory neuron spiking data from the Levenstein data set
from numpy import genfromtxt
def readData(filepath):
    """ I used this function to read in data so it could be segmented using Ann Christin's quartic polynomial functions in this file. Not used for anything else.
    
    filepath is the path to a csv file containing a column of population spiking frequencies, wiht each entry corresponding to a time interval of size typically 0.01 s
        for example,this could be the output of frequencyData_for_segmentation_function.m, saved in a csv file """
    my_data = genfromtxt(filepath, delimiter=',')
    my_data = my_data[1:] # for some reason it is putting NaN if I don't set this
    return my_data
...
def quartic_polynomial_fit(mov_avg,bins,save_path = None,saveplot = False):
    ...
    plt.plot(hist,label = "logarithmic histogram of moving average",color = '#348ABD',linewidth = 5)
    plt.show()
