...
import changedStatistics as stats_UD
...
def mov_avg_crossings_2thr_plot(sim_time,rate,mov_avg,crossings,window_avg,threshold_DU,threshold_UD,dt,start_time = None,end_time = None, start_down = False, show_legend = False,saveplot = False,save_path = None):
    ...
    if start_time != None and end_time != None:
        start_time = round(start_time/dt)
        end_time = round(end_time/dt)

    sim_time_cut = sim_time[start_time:end_time]
    #sim_time_cut = sim_time[start_time:end_time + 1]
    rate_cut = rate
    mov_avg_cut = mov_avg
    #crossings_cut = crossings
    #rate_cut = rate[start_time:end_time]
    #mov_avg_cut = mov_avg[start_time:end_time]
    ...
    # Testing lengths:
    print(len(sim_time_cut), len(rate_cut), len(mov_avg_cut))

    plt.plot(sim_time_cut,rate_cut,zorder=1,label = "Firing rate")
    plt.plot(sim_time_cut,mov_avg_cut, 'k-', linewidth = 4,zorder=2,label = f"Moving average",)
    ...
    # Shade the different states
    crossing_times = np.add(np.multiply(crossings, dt), start_time * dt)
    if start_down == False:
        #plt.axvspan(start_time * dt, crossing_times[0], facecolor='b', alpha=0.5)
        #plt.axvspan(crossing_times[-1], end_time * dt, facecolor='b', alpha=0.5)
        for i in range(0, len(crossing_times)-2, 2):
            plt.axvspan(crossing_times[i], crossing_times[i+1], facecolor='b', alpha=0.5)
            #plt.axvspan(crossing_times[i+1], crossing_times[i+2], facecolor='y', alpha=0.7)
    #else:
    #    for i in range(0, len(crossing_times)-1, 2):
    #        plt.axvspan(crossing_times[i], crossing_times[i+1], facecolor='y', alpha=0.5)
    #        plt.axvspan(crossing_times[i+1], crossing_times[i+2], facecolor='b', alpha=0.7)
